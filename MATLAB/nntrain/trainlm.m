...
if isempty(net.performFcn)%this is not affecting the performance function
%when it is set to one's own version of mse
...
function [worker,calcNet] = trainingIteration(worker,calcLib,calcNet)%not called anywhere in this file
...
while true
    ...
    perf2 = calcLib.trainPerf(calcNet2);%might be the mid-train performance function, not sure yet
